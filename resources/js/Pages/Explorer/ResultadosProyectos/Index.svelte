...
<AuthenticatedLayout>
    <div class="p-24 mt-10" style="background: #fff url(/storage/images/dots.png);background-size: cover;background-position: right;background-repeat: no-repeat;">
        ...
        <p class="pre-line-initial mt-3  text-gray-500 sm:mt-5 md:mt-5 lg:mx-0 text-center text-2xl w-full">
            {proyectos.length} resultado(s) para: {searchWord}
        </p>
    </div>
    <div class="mt-8">
        <div class="max-w-7xl mx-auto">
            <div class="py-[6rem]">
                ...
                <div class="grid grid-cols-{proyectos.length <= 3 ? proyectos.length : 3} justify-around gap-3">
                    {#each proyectos as proyecto}
                        <a use:inertia href={route('nodos.explorer.busqueda-proyectos.ver-proyecto', [nodo, proyecto.id])} class="text-white block m-2 {proyectos.length == 1 ? 'w-[28rem]' : ''}">
