<script>
    ...
    export let nodo
    export let ambientesFormacion
    export let search

    $title = 'Resultados de búsqueda de ambientes'
...
<AuthenticatedLayout>
    <div class="p-24 mt-10" style="background: #fff url(/storage/images/dots.png);background-size: cover;background-position: right;background-repeat: no-repeat;">
        ...
        <p class="pre-line-initial mt-3  text-gray-500 sm:mt-5 md:mt-5 lg:mx-0 text-center text-2xl w-full">
            {ambientesFormacion.length} resultado(s) para: {search}
        </p>
    </div>
    <div class="mt-8">
        <div class="max-w-7xl mx-auto">
            <div class="py-[6rem]">
                ...
                <div class="grid grid-cols-{ambientesFormacion.length <= 3 ? ambientesFormacion.length : 3} justify-around gap-3">
                    {#each ambientesFormacion as ambienteFormacion}
                        <a use:inertia href={route('nodos.explorer.busqueda-ambientes.ver-ambiente', [nodo, ambienteFormacion.id])} class="text-white block m-2 shadow-xl">
                            <div class="text-white">
                                <img src="/storage/{ambienteFormacion.foto}" class="w-full h-64 object-cover" alt="" />
                ...
                {#if ambientesFormacion.length == 0}
